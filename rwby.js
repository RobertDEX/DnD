// ============================================================
// RWBY DnD — rwby.js  |  Firebase + Uncanny Dodge calc
// ============================================================
...
const DOC_ID = 'rwby-campaign';

// ---- CONSTANTS ----
const DM_PASSWORD  = '123456789';
const STORAGE_KEY  = 'rwby-dnd-v3-local';
...
// ---- UNCANNY DODGE CALCULATION ----
// For every 10 damage → +1 AP cost.  < 20 dmg → 1 AP cost.
// Actual damage taken = Math.ceil(dmg / 2)
function calcUncannyDodge(dmg) {
  if (!dmg || dmg <= 0) return null;
  let apCost;
  if (dmg < 20) {
    apCost = 1;
  } else {
    apCost = Math.floor(dmg / 10);
  }
  const halved = Math.ceil(dmg / 2);
  return { apCost, halved };
}
window.calcUncanny = function() {
  const inp = document.getElementById('uncannyDmg');
...
async function pushState() {
  ...
  setSyncDot('syncing');
  try {
    await setDoc(doc(db, 'campaigns', DOC_ID), { data: JSON.stringify(state), updated: Date.now() });
    setSyncDot('synced');
...
function setSyncDot(status) {
  ...
  dot.title = status === 'synced' ? 'Synced to Firebase' : status === 'syncing' ? 'Syncing…' : 'Sync error — using local data';
}
...
function ensureClamp(c) {
  c.hp.max=Math.max(0,Number(c.hp.max)||0); c.aura.max=Math.max(0,Number(c.aura.max)||0);
...
function hexRgba(hex,a=1) {
  const cl=hex.replace('#',''); const r=parseInt(cl.slice(0,2),16),g=parseInt(cl.slice(2,4),16),b=parseInt(cl.slice(4,6),16);
  return `rgba(${r},${g},${b},${a})`;
}
...
function applyTheme() {
  ...
  root.style.setProperty('--aura',t.aura); root.style.setProperty('--text',t.text);
  root.style.setProperty('--panel',hexRgba(t.panel,.97)); root.style.setProperty('--line-hi',hexRgba(t.accent,.5));
  document.body.style.background=`radial-gradient(ellipse at 0% 0%,${hexRgba(t.accent,.18)} 0%,transparent 40%),radial-gradient(ellipse at 100% 100%,${hexRgba(t.accentTwo,.15)} 0%,transparent 40%),linear-gradient(180deg,#000 0%,${t.bg} 50%,${t.accentTwo} 100%)`;
}
function renderThemeFields() {
  ...
  f('themeAccentTwoColor',t.accentTwo);f('themeAuraColor',t.aura);f('themeTextColor',t.text);
}

// ---- RENDER FUNCTIONS ----
function renderCharacterTabs() {
  const tabs = el('characterTabs'); if(!tabs)return;
  tabs.innerHTML = '';
  state.characters.forEach((c,i) => {
    ...
    btn.innerHTML=`<strong>${esc(c.name||`Player ${i+1}`)}</strong><span>${esc(c.className||'—')} · Lv${c.level}</span><div class="tab-hp-bar"><div class="tab-hp-fill" style="width:${pct}%"></div></div>`;
...
function renderMainFields() {
  ...
  sv('charWeaponName',c.weaponName||'');sv('charPB',c.proficiencyBonus);
  sv('currentHp',c.hp.current);sv('maxHp',c.hp.max);
  sv('currentAura',c.aura.current);sv('maxAura',c.aura.max);
  sv('armor',c.armor);sv('initiative',c.initiative);sv('speed',c.speed);sv('tempHp',c.tempHp||0);
  ...
  const st=el('stateActive');if(st){el('stateActive').checked=c.state==='active';el('stateReserve').checked=c.state==='reserve';el('stateDead').checked=c.state==='dead';}
}
function renderStats() {
  ...
  STAT_ORDER.forEach(stat=>{
    ...
    card.innerHTML=`<div class="stat-key">${stat}</div><input class="stat-score-input" data-stat="${stat}" type="number" value="${score}"><div class="stat-mod">${fmtMod(mod)}</div><div class="stat-controls"><button type="button" data-stat="${stat}" data-action="minus">−</button><button type="button" data-stat="${stat}" data-action="plus">+</button></div>`;
    g.appendChild(card);
  });
  g.querySelectorAll('.stat-score-input').forEach(inp=>{
    inp.addEventListener('input',e=>{c.stats[e.target.dataset.stat]=Number(e.target.value)||0;pushState();renderStats();renderSkills();renderSemblance();});
  });
}
function renderSkills() {
  ...
  Object.entries(SKILL_MAP).forEach(([stat,skills])=>{
    ...
    skills.forEach(sk=>{
      const sd=c.skills[sk]||{stat,bonus:0};
      const total=sm+(Number(sd.bonus)||0);
  ...
  m.querySelectorAll('input[data-skill]').forEach(inp=>{
    inp.addEventListener('input',e=>{c.skills[e.target.dataset.skill].bonus=Number(e.target.value)||0;pushState();renderSkills();});
  });
}
function renderSemblance() {
  ...
  SEM_KEYS.forEach(key=>{
    ...
    card.innerHTML=`<details class="collapse-block"${locked?'':' open'}><summary class="collapse-summary"><div class="sem-stage-header"><div class="stage-name">${SEM_LABELS[key]}</div><span class="stage-badge${locked?'',' unlocked':' unlocked'}">${locked?'Locked':'Unlocked'}</span></div></summary><div class="collapse-body"><div class="stage-lines"><div class="stage-line"><strong>${esc(stage.active||'Active — Unnamed')}</strong><div>${esc(stage.activeDescription||'No description. Ask your DM.')}</div><div class="cost-note">Aura Cost: ${stage.auraCost}</div></div><div class="stage-line"><strong>${esc(stage.passiveName||'Passive — Unnamed')} [Passive]</strong><div>${esc(stage.passiveDescription||'No passive description.')}</div></div></div><div class="technique-actions"><button type="button" class="neo-btn small use-btn use-sem" data-stage="${key}"${locked?' disabled':''}>Use Active (−${stage.auraCost} Aura)</button></div></div></details>`;
...
function renderTechniques() {
  ...
  if(!c.techniques.length){cont.innerHTML=`<div style="padding:1rem;color:var(--muted)">No Aura Techniques granted yet. The DM assigns them via the DM panel.</div>`;return;}
  c.techniques.forEach(t=>{
    const card=document.createElement('div'); card.className='technique-card';
    card.innerHTML=`<details class="collapse-block"><summary class="collapse-summary"><div class="technique-top"><strong>${esc(t.name)}</strong><div class="technique-meta"><span class="meta-pill">Lv ${t.level}</span><span class="meta-pill aura">−${t.cost} Aura</span><span class="meta-pill">${esc(t.type)}</span></div></div></summary><div class="collapse-body"><div class="small-note">${esc(t.description)}</div><div class="technique-actions"><button type="button" class="neo-btn small use-btn use-tech" data-id="${t.id}">Use (−${t.cost} Aura)</button></div></div></details>`;
    cont.appendChild(card);
  });
...
function renderDust() {
  ...
  g.querySelectorAll('input[data-dust]').forEach(inp=>inp.addEventListener('input',e=>{c.dustInventory[e.target.dataset.dust]=Math.max(0,Number(e.target.value)||0);pushState();}));
...
function renderDustSpells() {
  ...
  c.dustSpells.forEach(sp=>{
    const card=document.createElement('div'); card.className='technique-card';
    card.innerHTML=`<details class="collapse-block"><summary class="collapse-summary"><div class="technique-top"><strong>${esc(sp.name)}</strong><div class="technique-meta"><span class="meta-pill">${esc(sp.type)}</span><span class="meta-pill">Consumes 1</span></div></div></summary><div class="collapse-body"><div class="small-note">${esc(sp.description)}</div><div class="dust-card-actions"><button type="button" class="neo-btn small use-btn use-dust" data-id="${sp.id}">Use (−1 ${esc(sp.type)})</button><button type="button" class="neo-btn small ghost del-dust" data-id="${sp.id}">Delete</button></div></div></details>`;
    cont.appendChild(card);
  });
  cont.querySelectorAll('.use-dust').forEach(btn=>btn.addEventListener('click',()=>useDustSpell(btn.dataset.id)));
  cont.querySelectorAll('.del-dust').forEach(btn=>btn.addEventListener('click',()=>{c.dustSpells=c.dustSpells.filter(s=>s.id!==btn.dataset.id);pushState();renderDust();}));
}
function renderDmSemblance() {
  ...
  SEM_KEYS.forEach(key=>{
    ...
    col.innerHTML=`<div class="section-label" style="margin-bottom:.6rem">${SEM_LABELS[key]}</div><div class="field"><label>Active Name</label><input type="text" data-sem="${key}" data-f="active" value="${esc(s.active)}"></div><div class="field"><label>Active Desc</label><textarea class="small-textarea" data-sem="${key}" data-f="activeDescription">${esc(s.activeDescription)}</textarea></div><div class="field"><label>Passive Name</label><input type="text" data-sem="${key}" data-f="passiveName" value="${esc(s.passiveName)}"></div><div class="field"><label>Passive Desc</label><textarea class="small-textarea" data-sem="${key}" data-f="passiveDescription">${esc(s.passiveDescription)}</textarea></div><div class="field"><label>Aura Cost</label><input type="number" min="0" data-sem="${key}" data-f="auraCost" value="${s.auraCost}"></div>`;
    g.appendChild(col);
  });
...
function renderDmTechniques() {
  ...
  // Show all characters' techniques grouped
  let any=false;
  state.characters.forEach(c=>{
    if(!c.techniques.length) return;
    any=true;
    const header=document.createElement('div');
    header.style.cssText='font-family:var(--font-mono);font-size:.72rem;letter-spacing:.15em;text-transform:uppercase;color:var(--aura);margin:.65rem 0 .4rem;padding:.4rem .6rem;background:rgba(80,150,255,.06);border-radius:6px;border-left:2px solid var(--aura)';
    header.textContent=c.name||`Player`;
    cont.appendChild(header);
    c.techniques.forEach(t=>{
      const card=document.createElement('div'); card.className='technique-card';
      card.innerHTML=`<details class="collapse-block"><summary class="collapse-summary"><div class="technique-top"><strong>${esc(t.name)}</strong><div class="technique-meta"><span class="meta-pill">Lv ${t.level}</span><span class="meta-pill aura">−${t.cost}</span><span class="meta-pill">${esc(t.type)}</span></div></div></summary><div class="collapse-body"><div class="form-grid"><div class="field"><label>Name</label><input type="text" data-cid="${c.id}" data-tid="${t.id}" data-f="name" value="${esc(t.name)}"></div><div class="field"><label>Level</label><input type="number" data-cid="${c.id}" data-tid="${t.id}" data-f="level" value="${t.level}"></div><div class="field"><label>Cost</label><input type="number" data-cid="${c.id}" data-tid="${t.id}" data-f="cost" value="${t.cost}"></div><div class="field"><label>Type</label><input type="text" data-cid="${c.id}" data-tid="${t.id}" data-f="type" value="${esc(t.type)}"></div></div><div class="field"><label>Description</label><textarea class="small-textarea" data-cid="${c.id}" data-tid="${t.id}" data-f="description">${esc(t.description)}</textarea></div><div class="dm-tech-actions"><button type="button" class="neo-btn small ghost" data-del-cid="${c.id}" data-del-tid="${t.id}">Delete</button></div></div></details>`;
      cont.appendChild(card);
    });
  });
  ...
  cont.querySelectorAll('[data-cid][data-tid][data-f]').forEach(inp=>{
    inp.addEventListener('input',e=>{
      ...
      if(!t)return; const f=e.target.dataset.f;
...
function render() {
  ...
  renderCharacterTabs();renderHeader();renderMainFields();renderStats();renderSkills();
  renderSemblance();renderTechniques();renderDust();renderDmSemblance();renderDmTechniques();
  renderDmTargetSelect();renderThemeFields();renderTabs();
}

// ---- ACTIONS ----
function adjustResource(resource,amount) {
  const c=getChar();
  if(resource==='hp')  c.hp.current=clamp(c.hp.current+amount,0,c.hp.max);
  if(resource==='aura')c.aura.current=clamp(c.aura.current+amount,0,c.aura.max);
  pushState();renderHeader();renderMainFields();renderCharacterTabs();
}
function useTechnique(id) {
  const c=getChar(); const t=c.techniques.find(t=>t.id===id);
  if(!t)return;
...
function useSemblance(key) {
  const c=getChar();
  if(stageLocked(key,c)){alert('That stage is locked.');return;}
...
function useDustSpell(id) {
  const c=getChar(); const sp=c.dustSpells.find(s=>s.id===id);
  if(!sp)return;
  if((c.dustInventory[sp.type]||0)<1){alert(`Not enough ${sp.type}.`);return;}
  c.dustInventory[sp.type]--; pushState(); renderDust();
}
function rollHp() {
  const c=getChar(); const conMod=modFrom(c.stats.CON); const roll=rollD10();
  const total=Math.max(1,roll+conMod); c.hp.max+=total; c.hp.current=c.hp.max;
  pushState(); render(); alert(`HP Roll: d10 (${roll}) + CON (${conMod}) = +${total} HP\nNew Max: ${c.hp.max}`);
}
function rollAura() {
  const c=getChar(); const bonus=c.skills['Aura Mastery']?.bonus||0; const roll=rollD10();
  const total=Math.max(1,roll+bonus); c.aura.max+=total; c.aura.current=c.aura.max;
  pushState(); render(); alert(`Aura Roll: d10 (${roll}) + Aura Mastery (${bonus}) = +${total}\nNew Max: ${c.aura.max}`);
}
...
function createTechnique() {
  ...
  const targetIdx=Number(el('dmTechTarget')?.value??state.selectedCharacter);
  if(!name||!type||!desc){alert('Fill out all technique fields.');return;}
  const target=state.characters[targetIdx];
  if(!target){alert('No target character found.');return;}
...
function exportSave() {
  ...
  const a=document.createElement('a'); a.href=url; a.download=`rwby-save-${new Date().toISOString().slice(0,10)}.json`;
...
function openDmOverlay() {
  ...
  else{el('dmLoginPanel')?.classList.add('hidden');el('dmFullscreenPanel')?.classList.remove('hidden');renderDmSemblance();renderDmTechniques();renderDmTargetSelect();renderThemeFields();}
}
...
function updateField(field,value) {
  ...
  else if(['level','proficiencyBonus','armor','tempHp'].includes(field))c[field]=Math.max(0,Number(value)||0);
  else c[field]=value;
  ensureClamp(c); pushState(); renderHeader(); renderMainFields(); renderCharacterTabs();
}
function bindInputs(){
  ...
  ii('charWeaponName','weaponName');ii('charPB','proficiencyBonus');
  ii('currentHp','currentHp');ii('maxHp','maxHp');ii('currentAura','currentAura');ii('maxAura','maxAura');
  ii('armor','armor');ii('initiative','initiative');ii('speed','speed');ii('tempHp','tempHp');
  ...
  el('saveThemeBtn')?.addEventListener('click',()=>{state.theme={bg:el('themeBgColor')?.value,panel:el('themePanelColor')?.value,accent:el('themeAccentColor')?.value,accentTwo:el('themeAccentTwoColor')?.value,aura:el('themeAuraColor')?.value,text:el('themeTextColor')?.value};pushState();render();});
  el('resetThemeBtn')?.addEventListener('click',()=>{state.theme={...DEF_THEME};pushState();render();});
  [el('themeBgColor'),el('themePanelColor'),el('themeAccentColor'),el('themeAccentTwoColor'),el('themeAuraColor'),el('themeTextColor')].forEach(inp=>{if(inp)inp.addEventListener('input',()=>{state.theme={bg:el('themeBgColor')?.value||DEF_THEME.bg,panel:el('themePanelColor')?.value||DEF_THEME.panel,accent:el('themeAccentColor')?.value||DEF_THEME.accent,accentTwo:el('themeAccentTwoColor')?.value||DEF_THEME.accentTwo,aura:el('themeAuraColor')?.value||DEF_THEME.aura,text:el('themeTextColor')?.value||DEF_THEME.text};applyTheme();});});
}
